...
import {getAlbums, getPublishedBlogsByUserId} from "@/utils/api";
...
export default {
    ...
    methods: {
        // 获取博客列表
        async getBlogs() {
            const res = await getPublishedBlogsByUserId(this.$route.params.userId ,this.blogQuery);
            console.log(res);
        ...
        // 获取相册列表
        async getUserAlbums() {
            const res = await getAlbums(this.$route.params.userId);
            console.log(res);
